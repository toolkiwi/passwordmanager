...
export const UnlockVault = () => {
     ...
     * On unlock volt
     */
    const handleUnlockVault = async (e: React.ChangeEvent<HTMLFormElement>): Promise<void> => {
        e.preventDefault();
        e.stopPropagation();
        if (!file) return;
            ...
            {!file ? (
                    ...
                    >
                        <div className={CN.dropzone_meta}>
                            <div className={CN.dropzone_meta_icon}>
                                <TbFolderOpen className={CN.dropzone_meta_icon_svg} />
                            </div>
                            <div className={CN.dropzone_meta_title}>
                                {t('page:index.drag_ciphered_file')}{' '}
                                <span className={CN.dropzone_meta_title_span}>(.ptk)</span>
                            </div>
                        </div>
                        <div className={CN.dropzone_or}>
                            <div className={CN.dropzone_or_separator}></div>
                            <div className={CN.dropzone_or_span}>{t('page:index.or')}</div>
                            <div className={CN.dropzone_or_separator}></div>
                        </div>
                        <div className={CN.dropzone_browse}>
                            <div className={CN.dropzone_browse_button}>{t('page:index.browse_files')}</div>
                        </div>
                    </label>
                ...
                <form onSubmit={handleUnlockVault}>
                    <div className='p-10 border border-solid  mb-5 rounded-lg max-sm:p-5'>
                        <label className='text-foreground font-semibold' htmlFor='vault-master'>
                        ...
                        <StyledButton button={{ type: 'submit', className: 'p-3 w-full' }} variant='secondary'>
...
const CN = {
    ...
    dropzone_meta_title: 'text-foreground font-bold text-xl text-center normal-case max-sm:text-lg',
    ...
    dropzone_or: 'py-8 relative w-full text-center uppercase flex flex-row items-center max-sm:py-5',
    dropzone_or_span: 'px-3 z-10',
    dropzone_or_separator: 'flex-1 h-[1px] bg-white/10',
    dropzone_browse: 'mb-5',
    dropzone_browse_button: 'bg-white/10 rounded-lg py-3 px-5 text-gray-400 text-sm font-extrabold normal-case',
};
