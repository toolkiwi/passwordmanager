/**
 * Shared CN styles for forms
 */
export default {
    form: 'w-full h-full flex flex-col flex-1',
    input_group: 'flex flex-col',
    container: 'p-5 flex flex-col gap-5',
    favicon_container:
        'relative w-12 h-12 flex items-center justify-center bg-black/80 p-2 border-1 border-neutral-900 rounded-lg',
    favicon: 'w-full h-full rounded-md',
    label: 'text-white text-sm font-bold uppercase',
    sub_label: 'text-neutral-600 mt-1 mb-2 text-md',
    footer: 'p-5 flex flex-row items-center gap-5',
};
